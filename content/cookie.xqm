...
declare %private function cookie:name-and-value($name as xs:string?, $value as xs:string?) as xs:string {
    ...
    ) else if (matches($name, $cookie:enforce-rfc2109)) then (
...
declare %private function cookie:lifetime($max-age as item()?) as xs:string* {
    ...
    ) else if (not($max-age instance of xs:dayTimeDuration) and not($max-age castable as xs:integer)) then (
        error(
            $errors:OPERATION,
            "cookie:set: max-age must be an instance of xs:dayTimeDuration or xs:integer",
            $max-age
        )
    ) else if ($max-age instance of xs:integer) then (
        "Max-Age=" || $max-age, "Expires=" || string(current-dateTime() + xs:dayTimeDuration("PT" || $max-age || "S"))
    ) else (
        "Max-Age=" || ($max-age div xs:dayTimeDuration("PT1S")), "Expires=" || string(current-dateTime() + $max-age)
    )
};
...
declare %private function cookie:samesite($samesite as xs:string?) as xs:string? {
    if (empty($samesite)) then (
        ...
        error(
            $errors:OPERATION,
            "cookie:set: SameSite must be one of " || string-join($cookie:properties?samesite, ", "),
            $samesite
        )
    ) else (
        "SameSite=" || $samesite
...
declare %private function cookie:add-property($options as map(*), $property as xs:string) as xs:string? {
    if (empty($options?($property))) then (
    ) else (
...
declare %private function cookie:add-flag($options as map(*), $property as xs:string) as xs:string? {
    if (boolean($options?($property))) then (
        $cookie:capitalized?($property)
    ) else (
    )
};
