...
declare function body:parse($request as map(*)) {
    ...
    ) (: this route expects no body, return an empty sequence :) else (
        try {
            ...
            error(
                ...
                "Body with media type '" ||
                    $request?media-type ||
                    "' could not be parsed (invalid " ||
                    upper-case($request?format) ||
                    ").",
                $err:description
            )
        }
...
declare function body:content-type($request as map(*)) as map(*) {
    if (not(exists($request?config?requestBody?content))) then (
        map {}
    ) (: this route expects no body, return an empty map :) else if (
        not($request?config?requestBody?content instance of map(*))
    ) then
        ...
        let $media-type := if (contains($raw-content-type-header-value, ";")) then
            substring-before($raw-content-type-header-value, ";")
        else
            $raw-content-type-header-value

        let $charset := if (contains($raw-content-type-header-value, "charset=")) then (
        ...
        return if (
            ...
            map {
                ...
                "format":
                    if ($media-type = ("application/json", "text/json")) then
                        "json"
                    else if ($media-type = ("application/xml", "text/xml", "image/svg+xml")) then
                        "xml"
                    else if ($media-type = ("multipart/form-data", "application/x-www-form-urlencoded")) then
                        "form-data"
                    else if (
                        starts-with($media-type, "application/") and
                            (ends-with($media-type, "+json") or ends-with($media-type, "+xml"))
                    ) then
                        substring-after($media-type, "+")
                    else
                        "binary"
            }
        else
            error($errors:BODY_CONTENT_TYPE, "Body with media-type '" || $media-type || "' is not allowed", $request)
    )
};

declare %private function body:get-form-data-value($name as xs:string, $format as xs:string?) as item()* {
    switch ($format)
        case "binary" return
            if (request:is-multipart-content()) then
                ...
                return for $_name at $index in $names
                    return map {"name": $_name, "data": $data[$index], "size": $sizes[$index]}
            else
...
declare %private function body:additional-property($name as xs:string) as map(*) {
    map {$name: request:get-parameter($name, ())}
};

declare %private function body:validate-value($schema as map(*)) as function(*) {
    let $required-props := $schema?required?*
    let $property-definitions := $schema?properties
    return function ($name as xs:string) as map(*) {
        if (not(map:contains($property-definitions, $name))) (: additional property, no validation :) then
            body:additional-property($name)
        else
            ...
            let $format := if ($property?type = "array") then
                $property?items?format
            else
                $property?format
            let $value := body:get-form-data-value($name, $format)
            return if (not(exists($value)) and $is-required) then
                error($errors:BAD_REQUEST, 'Property "' || $name || '" is required!')
            else if (count($value) > 1 and not($is-array)) then
                error(
                    $errors:BAD_REQUEST,
...
(:~
 ...
 :)
declare %private function body:ensure-required-properties(
    $received-property-names as xs:string*,
    $required-properties as xs:string*
) as xs:string* {
    for-each(
        $required-properties,
        function ($required-prop-name as xs:string) as empty-sequence() {
            if ($required-properties = $received-property-names) then (
            ) else
                error($errors:BAD_REQUEST, 'Property "' || $required-prop-name || '" is required!')
        }
    ),
...
(:~
 ...
 :)
declare %private function body:parse-form-data($schema as map(*)?) as map(*) {
    if (exists($schema)) then
        map:merge(
            for-each(
                body:ensure-required-properties(request:get-parameter-names(), $schema?required?*),
