...
declare function parameters:in-path($request as map(*), $response as map(*)) as map(*)+ {
    ...
    return if (not($has-path-parameters-in-pattern) and exists($path-param-map)) then
        error(
            $errors:OPERATION,
            "Path pattern has no substitutions, but path parameters are defined " || $request?pattern,
            $request
        )
    else if (not($has-path-parameters-in-pattern)) then (
        $request, $response
    ) (: the matching route does not define path parameters :) else
        ...
        let $maps :=
            ...
            return if (map:contains($path-param-map, $key)) then (
        ...
        return (map:put($request, "parameters", $merged), $response)
};
...
declare %private function parameters:get-path-parameter-map-from-config($parameters as array(*)?) as map(*)? {
    if (not(exists($parameters))) then (
    ) (: no parameters defined :) else
        let $path-parameters := for-each(
            $parameters?*,
            function ($parameter as map(*)) as map(*)? {
                if (parameters:is-path-parameter($parameter)) then
        ...
        return if (count($path-parameters)) then
            map:merge($path-parameters)
        else (
        )
};
...
declare function parameters:in-request($request as map(*), $response as map(*)) as map(*)+ {
    if (not(map:contains($request?config, "parameters"))) then (
        $request, $response
    ) (: route expects no parameters, return request unchanged :) else if (
        not($request?config?parameters instance of array(*))
    ) then
        error($errors:OPERATION, "Parameter definition must be an array: " || $request?pattern, $request)
    else
...
declare %private function parameters:retrieve($parameter as map(*)) as map(*)? {
    if (parameters:is-path-parameter($parameter)) then (
        ...
        let $values := switch ($parameter?in)
            case "header" return
        ...
        return if ($parameter?required and empty($cast)) then (
            error(
                $errors:REQUIRED_PARAM,
                "Required " || $parameter?in || '-parameter "' || $name || '" missing or empty.'
            )
        ) else (
            map {$name: $cast}
        )
        ...
        case "object" return
            error(
                ...
                "The " ||
                    ...
                    '-parameter "' ||
                    $config?name ||
                    '" is of type "object", which is not supported yet.'
            )
        case "array" return
...
declare %private function parameters:cast-atomic($values as xs:string*, $config as map(*)) as item()? {
    if (count($values) > 1) then
        error(
            $errors:BAD_REQUEST,
            "Multiple values were provided for " ||
                $config?in ||
                '-parameter "' ||
                $config?name ||
                '", which is not declared an array.'
        )
    else if (count($values) = 1) then
        parameters:cast-value($values, $config?schema)
    else
        parameters:get-parameter-default-value($config?schema)!parameters:cast-value(., $config?schema)
};

declare %private function parameters:cast-array($values as xs:string*, $config as map(*)) as array(*)? {
    let $style := if (exists($config?style) and $config?style = ("label", "matrix")) then (
        (: do not throw but log on debug :)
        error(
            $errors:OPERATION,
            "Unsupported style " || $config?style || " for " || $config?in || '-parameter "' || $config?name || '".'
        )
    ...
    let $tokenized-values := if (empty($values) and empty($default)) then (
        (: null :)
    ) else if (empty($values)) then (
        $default?*
    ) else if (
        $explode and ($config?in eq "cookie" or $config?style = ("spaceDelimited", "pipeDelimited", "simple"))
    ) then (
        error(
            $errors:OPERATION,
            "Explode cannot be true for " ||
                $config?in ||
                '-parameter "' ||
                $config?name ||
                '" with style set to ' ||
        ...
        error(
            ...
            "Multiple entries for " ||
                ...
                '-parameter "' ||
                $config?name ||
                '" found but explode is set to false.'
        )
    ) else (
        let $separator := switch ($style)
            ...
            case "pipeDelimited" return
                "\|" (: pipe needs to be escaped for use in tokenize :)
