...
describe('public route with custom middleware', function () {
    ...
    it('should return with access control headers', function () {
        expect(publicRouteResult.headers['access-control-allow-origin']).to.equal('*')
        expect(publicRouteResult.headers['access-control-allow-credentials']).to.equal('true')
        expect(publicRouteResult.headers['access-control-allow-methods']).to.equal(
            'GET, POST, DELETE, PUT, PATCH, OPTIONS'
        )
        expect(publicRouteResult.headers['access-control-allow-headers']).to.equal(
            'Accept, Content-Type, Authorization, X-Auth-Token'
        )
    })
})
