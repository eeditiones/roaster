...
declare function jwt-auth:issue-token($request as map(*)) {
    if (
        ...
        return if ($loggedin and $username = $user?name) then (
            router:response(201, (), map {"user": $user, "token": $jwt-auth:jwt?create($user)}, ())
        ) else
...
declare function jwt-auth:bearer-auth($request as map(*)) as map(*)? {
    try {
        ...
        return if (exists($token)) then (
            let $payload := $jwt-auth:jwt?read($token)
            return map {"name": $payload?name, "groups": $payload?groups, "dba": $payload?dba}
        ) else (
        )
    } catch too-old {
    ...
    }catch * { error($errors:SERVER_ERROR, "Server error") }
};
