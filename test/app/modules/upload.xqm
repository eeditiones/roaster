...
declare function upload:single($request as map(*)) {
    try {
        ...
        let $stored as xs:string := xmldb:store($upload:collection, $file-name, $file?data)

        return roaster:response(201, map {"uploaded": $upload:download-path || $file-name})
    } catch * { roaster:response(400, map {"error": $err:description}) }
};
...
declare function upload:batch($request as map(*)) {
    try {
        ...
        return roaster:response(201, map {"uploaded": array { $download-links }})
    } catch * { roaster:response(400, map {"error": $err:description}) }
};
...
declare function upload:base64($request as map(*)) {
    ...
    let $stored as xs:string := xmldb:store("/db/apps/roasted/uploads", $file-name, $file-content)
